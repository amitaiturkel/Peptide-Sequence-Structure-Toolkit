...
from binding_predictor_esm_without_weight import (
    BindingPredictor,
    load_esm_model,
    get_esm_embeddings,
)
...
MODEL_PATH   = os.path.join(
    PROJECT_ROOT,
    "saved_models",
    "model_emb_best_without_weight.pt",
)
...
class SequencePredictorApp:
    def __init__(self, root):
        ...
        self.esm_model, self.alphabet, self.batch_converter, self.device = \
            load_esm_model(embedding_size=1280)
    ...
    def _load_classifier(self, path):
        model = BindingPredictor(emb_dim=1280, hidden_dim=128)
