...
if __name__ == '__main__':
    ...
    dir_path = os.path.dirname(results_csv_path)
    if dir_path:
    ...
    with open(results_csv_path, mode="a", newline="") as csv_file:
        ...
        if write_header:
            csv_writer.writerow([
                "embedding_size", "embedding_layer", "test_size",
                "batch_size", "epochs", "lr", "hidden_dim", "dropout",
                "AUC"
            ])
    embedding_sizes_layers = [(320,6),(480,12),(640,30),(1280,33),(2560,36),(5120,48)] #each tubple repesnt a model
    test_sizes = [0.2, 0.25, 0.3]

    # Neural net parameter grid
    ...
    for embedding_size_layer, test_size in product(embedding_sizes_layers, test_sizes):
        embedding_layer = embedding_size_layer[1]
        embedding_size = embedding_size_layer[0]
        print(f"\n🧪 Trying ESM config: embedding_size={embedding_size}, layer={embedding_layer}, test_size={test_size}")
        chosen_embedding_layer = embedding_layer
        chosen_embedding_size =embedding_size
        chosen_test_size = test_size
    ...
    with open(results_csv_path, mode="a", newline="") as csv_file:
        ...
        csv_writer.writerow(["BEST CONFIG"])
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow([
            best_config["embedding_size"], best_config["embedding_layer"], best_config["test_size"],
            best_config["batch_size"], best_config["epochs"], best_config["lr"],
